...
# Configuration
DATA_PATH = r"C:\Users\Prakhar\Desktop\New Folder\Data"  # Update this path
...
def prompt():
    ...
    return f"Suggestion: {results['answer']}" 
...
def crop_recom():
    ...
    nitrogen = res['Nitrogen']
    k = res['K']
    p = res['P']
    ph = res['pH']
    lat=res['latitude']
    long=res['longitude']
    wea=getWeather(lat,long)
    X_test=[nitrogen,p,k,wea['hourly_temperatures'],wea['current_humidity'],ph,wea['rainfall']]
...
@app.get("/getCropYield")
def yield_pred():
    reg = joblib.load('best_model.joblib')
    # get Area , Item  hectares, rainfall,pesticide_tonnes,avg tonnes from db
    farmer_id=request.args.get('uid', type=str, default=0)
    res = (
    supabase_extension.client
    .from_('user')
    ...
    area = res['Area']
    pesticide_usage = res['Pesticide_Usage']
    crop = res['Crop']
    nitrogen = res['Nitrogen']
    k = res['K']
    p = res['P']
...
# Function to get current location's latitude and longitude
    ...
    R=getWeather(get_current_location()[0],get_current_location()[1])
